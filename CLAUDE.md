...
## Design system
Dark theme only (Tailwind v4 CSS variables in app/globals.css) — a warm charcoal ground with an iron-amber accent, grounded in gym materiality rather than a generic dark-mode default. Display font: Oswald (headings, h1-h4 auto-styled via the base layer). Body font: Inter. Change the gym's name/tagline in lib/site.ts — that's the one place to edit for a different client. The barbell-divider CSS class in globals.css is the intentional signature element; used sparingly (home hero only) — don't scatter it everywhere.
